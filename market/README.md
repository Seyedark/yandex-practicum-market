...
Авторизация:
Реализована авторизация по логину и паролю через базовую форму. Пользователи заранее созданы в БД

Авторизованным пользователям доступен весь функционал, анонимным пользователям:

-Просмотр ленты товаров с постраничной навигацией и фильтрацией

-Просмотр отдельного товара

Приложение взаимодействует с сервисом платежей через OAuth2. Для корректной работы необходимо иметь 
настроенный keycloak и прописать необходимые конфигурации в приложении

Кеширование:

Реализовано кеширование товаров на ленте товаров, просмотра единичного товара, просмотра корзины
...
Приложение зависимо от приложения payment т.к. в нём находиться файл payment.yaml, который отвечает за генерацию клиента
...
Запустить исполняемый jar из консоли java -jar market.jar
